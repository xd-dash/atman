...
variable "tenants" {
  description = <<-EOT
    ...
    For each entry:
      ...
      - function_name / function_generation: this tenant's Cloud Function,
        so this module can also grant that function's own invoker role to
        the tenant's service account (roles/run.invoker for "gen2",
        roles/cloudfunctions.invoker for "gen1"). Leave function_name
        empty to skip this and manage that grant elsewhere.
  ...
  type = map(object({
    ...
    function_generation     = optional(string, "gen2")
    api_key                  = string
  }))
  default   = {}
  sensitive = true
  ...
  validation {
    condition = alltrue([
      for t in values(var.tenants) : contains(["gen1", "gen2"], t.function_generation)
    ])
    error_message = "function_generation must be \"gen1\" or \"gen2\"."
  }
}
